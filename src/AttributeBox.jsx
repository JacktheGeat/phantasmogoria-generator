...
function AttributeBox(setJSON) {
  ...
  function handleSetAttributes(target, newValue) {
    ...
    if ( newValue == undefined) {
      ...
      setJSON('attributes', {...everythingElse, [target] : 'undefined'});
      setAttributeList(items => ({...everythingElse}))
    }
    else {
  ...
  const displayMemo = useMemo(() => display(), [selectedValue, attributeList]);
  
  return {display: displayMemo}
}